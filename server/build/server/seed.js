...
import { db } from "./db";
import { categories, products, users, orders, orderItems, cartItems, } from "./schema";
...
async function seed() {
    ...
    const categoryMap = Object.fromEntries(categoryRecords.map((cat) => [cat.slug, cat.id]));
    ...
    const userData = await Promise.all(rawUsers.map(async (u) => ({
        name: u.name,
        email: u.email,
        phone: u.phone,
